...
The `build-release` job only runs when `releases_created == true`. It builds zips, generates PDFs, creates a CycloneDX SBOM for the npm dependency inventory, and publishes all of those artifacts — including the SBOM — to the GitHub release via `ncipollo/release-action`.
