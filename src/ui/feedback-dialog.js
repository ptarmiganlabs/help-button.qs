...
export function openFeedbackDialog(config, platformType) {
    ...
    const authStrategy = normalizeAuthStrategy(rawAuthStrategy, 'feedback.authStrategy');
    ...
    gatherContextData(allFields, platformType, dialogTimestampFormat).then((context) => {
        ...
        if (showPayloadButton) {
            ...
            showPayloadBtn.className = 'hbqs-bug-report-btn hbqs-bug-report-btn-show-payload';
            showPayloadBtn.type = 'button';
            showPayloadBtn.innerHTML =
                makeSvg('code', 14, '#374151') + '<span>' + escapeHtml(showPayloadText) + '</span>';
        ...
        submitBtn.innerHTML =
            makeSvg('send', 14, '#ffffff') + '<span>' + escapeHtml(submitText) + '</span>';
        ...
        function updateSubmitState() {
            ...
            const hasComment =
                enableComment && commentTextarea && commentTextarea.value.trim().length > 0;
...
function showDialogToast(toastArea, message, type) {
    ...
    toastArea.className =
        'hbqs-bug-report-toast hbqs-bug-report-toast-' + type + ' hbqs-bug-report-toast-visible';
}
 ...
 */
function buildPayload(context, config, payloadFields, rating, comment, timestampFormat) {
