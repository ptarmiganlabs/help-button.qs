...
| -------- | ---- | ------- | -------- | -------------- |
| **Medium** | Build dependencies | `npm audit` currently reports **6 moderate** vulnerabilities in dev/build tooling, all in the `@nebula.js/cli-serve` → `webpack-dev-server` → `sockjs` → `uuid` chain. The shipped extension bundle is not affected directly, but CI and local build environments still trust that toolchain. | `package.json`, `package-lock.json`, `npm audit` on 2026-05-28 | Track upstream Nebula/webpack updates and bump as soon as a patched dependency path is available. Re-run `npm audit` after each toolchain update. |
