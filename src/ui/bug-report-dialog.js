...
export function openBugReportDialog(config, platformType) {
    ...
    const authStrategy = normalizeAuthStrategy(rawAuthStrategy, 'bugReport.authStrategy');
    ...
    gatherContextData(allFields, platformType, dialogTimestampFormat).then((context) => {
        ...
        if (enableSeverity) {
            ...
            severityLabel.textContent =
                resolveText(dialogStrings.severityLabel, 'bugReportSeverityLabel') || 'Severity';
            ...
            const SEVERITY_OPTIONS = [
                {
                    value: 'low',
                    label:
                        resolveText(dialogStrings.severityLowLabel, 'bugReportSeverityLowLabel') ||
                        'Low',
                    color: '#22c55e',
                },
                {
        ...
        if (showPayloadButton) {
            const showPayloadText =
                resolveText(dialogStrings.showPayloadButton, 'bugReportShowPayload') ||
                'Show payload';
