...
import { isMenuItemVisible } from "../util/menu-item-merge";
...
export function createPopupMenu(triggerButton, config) {
  ...
  visibleMenuItems.forEach((item, idx) => {
    ...
    if (itemIcon)
      menuItem.style.setProperty("--hbqs-item-icon-color", itemIcon);
    ...
    if (item.action === "bugReport" && typeof onBugReport === "function") {
      ...
      if (!isSafeUrl(itemUrl)) {
        ...
        menuItem.setAttribute("aria-disabled", "true");
        logger.warn(
          "HelpButton.qs popup: blocked unsafe URL scheme for menu item:",
          item.label || "",
        );
        ...
        menuItem.addEventListener("click", (e) => {
          ...
          if (isSafeUrl(resolved)) {
            ...
            logger.warn(
              "HelpButton.qs popup: blocked unsafe resolved URL for menu item:",
              item.label || "",
            );
          }
        });
  ...
  if (popupStyle.borderRadius)
    popup.style.borderRadius = popupStyle.borderRadius;
