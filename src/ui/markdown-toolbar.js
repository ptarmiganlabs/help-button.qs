...
function applyAction(textarea, action) {
  ...
  const linePrefix = (prefix) => {
    ...
    const allPrefixed = lines.every((l) =>
      isOl ? olPat.test(l) : l.startsWith(prefix),
    );
    let replaced;
    if (allPrefixed) {
      replaced = lines
  ...
  switch (action) {
    ...
    case "hr": {
      ...
      textarea.selectionStart = textarea.selectionEnd =
        start + before.length + rule.length;
      textarea.focus();
      textarea.dispatchEvent(new Event("input", { bubbles: true }));
      break;
    }
...
export function createTabbedMarkdownEditor({
  ...
  textarea.className =
    "hbqs-md-tabbed-textarea" + (className ? " " + className : "");
  ...
  previewPanel.className =
    "hbqs-md-tabbed-panel hbqs-md-tabbed-panel--preview hbqs-markdown-content";
  previewPanel.style.display = "none";

  function updatePreview() {
    previewPanel.innerHTML =
      markdownToHtml(textarea.value, { allowedUriPatterns }) ||
